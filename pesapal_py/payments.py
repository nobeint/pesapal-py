...
class PesaPal(object):
    ...
    def get_transaction_status(self, token: str, order_tracking_id: str) -> dict:
        ...
        returns:
            status (str): success, pending, failed, or unknown (always returned)
            error (int): error code when getting transaction status fails
            message (str): a brief description about the response received (returned only when status is failed or unknown)
        """
        try:
            self.headers["Authorization"] = f"Bearer {token}"
            url = f"{self.base_production_url}/Transactions/GetTransactionStatus?orderTrackingId={order_tracking_id}"
            response = requests.get(url, headers=self.headers)
            response.raise_for_status()  # Raise HTTPError for bad responses

            transaction_status_data = response.json()

            if "status" not in transaction_status_data:
                return {
                    "status": "unknown",
            ...
            status_code = transaction_status_data.get("status", "")

            if status_code == "200":
                payment_status = transaction_status_data.get("payment_status_description", "")
                ...
                if payment_status == "Completed":
                    ...
                    return {
                        "status": "failed",
                        "error": transaction_status_data.get("error", {}).get("code", 0),
            ...
            return {
                "status": "failed",
                "error": transaction_status_data.get("error", {}).get("code", 0),
                "message": transaction_status_data.get("error", {}).get("message", "Unknown error"),
            }
